...
void free_device_packed_dataset(DevicePackedDataset& dds) {
    if (dds.d_items) cudaFree(dds.d_items);
    if (dds.d_start) cudaFree(dds.d_start);
    if (dds.d_len)   cudaFree(dds.d_len);
...
// one thread = one individual
// MSUS-like: split population into thirds, each third uses different search range
__global__ void search_kernel(
    ...
    int limit = max(1, static_cast<int>(ratio * m));
    if (limit > m) limit = m;

    unsigned int x = (pid ^ seed ^ (iter * 2654435761u));
    x = lcg_next(x);
    int pos = static_cast<int>(x % limit);
...
__global__ void support_fitness_kernel(
    const uint8_t* pop,
    const int* pop_count,
    int* fitness,
    const int* items,
...
__global__ void merge_best_kernel(
    uint8_t* base_pop,
    int* base_count,
    int* base_fit,
    ...
    if (best_src == 1) {
        src_pop = pop1; src_count = count1; src_fit = fit1;
    } else if (best_src == 2) {
        src_pop = pop2; src_count = count2; src_fit = fit2;
    } else {
        src_pop = pop3; src_count = count3; src_fit = fit3;
    }
...
void run_gpu_fim(
    ...
    uint8_t *d_pop = nullptr, *d_pop1 = nullptr, *d_pop2 = nullptr, *d_pop3 = nullptr;
    int *d_count = nullptr, *d_count1 = nullptr, *d_count2 = nullptr, *d_count3 = nullptr;
    ...
    support_fitness_kernel<<<fit_grid, fit_block>>>(
        d_pop, d_count, d_fit,
        dds1.d_items, dds1.d_start, dds1.d_len,
        N, m
    );
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        CUDA_CHECK(cudaMemcpy(d_count1, d_count, P * sizeof(int), cudaMemcpyDeviceToDevice));
        CUDA_CHECK(cudaMemcpy(d_count2, d_count, P * sizeof(int), cudaMemcpyDeviceToDevice));
        CUDA_CHECK(cudaMemcpy(d_count3, d_count, P * sizeof(int), cudaMemcpyDeviceToDevice));
        ...
        // search step
        search_kernel<<<pop_grid, pop_block>>>(d_pop1, d_count1, P, m, ALPHA, BETA, GAMMA, 101u, it);
        search_kernel<<<pop_grid, pop_block>>>(d_pop2, d_count2, P, m, ALPHA, BETA, GAMMA, 202u, it);
        search_kernel<<<pop_grid, pop_block>>>(d_pop3, d_count3, P, m, ALPHA, BETA, GAMMA, 303u, it);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());

        // fitness step
        support_fitness_kernel<<<fit_grid, fit_block>>>(
            d_pop1, d_count1, d_fit1,
            dds1.d_items, dds1.d_start, dds1.d_len,
            N, m
        );
        support_fitness_kernel<<<fit_grid, fit_block>>>(
            d_pop2, d_count2, d_fit2,
            dds2.d_items, dds2.d_start, dds2.d_len,
            N, m
        );
        support_fitness_kernel<<<fit_grid, fit_block>>>(
            d_pop3, d_count3, d_fit3,
            dds3.d_items, dds3.d_start, dds3.d_len,
            N, m
        );
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    // optional: copy base fitness back for a quick summary
    vector<int> h_fit(P, 0);
    CUDA_CHECK(cudaMemcpy(h_fit.data(), d_fit, P * sizeof(int), cudaMemcpyDeviceToHost));

    int best = 0;
    for (int v : h_fit) best = max(best, v);
    ...
    if (d_count1) cudaFree(d_count1);
    if (d_count2) cudaFree(d_count2);
    if (d_count3) cudaFree(d_count3);

    if (d_fit)  cudaFree(d_fit);
    if (d_fit1) cudaFree(d_fit1);
    if (d_fit2) cudaFree(d_fit2);
    if (d_fit3) cudaFree(d_fit3);
}